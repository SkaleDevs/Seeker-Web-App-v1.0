...
const Home = ({sess}) => {
  ...
  const clickHanlder1 = async() =>{
    ...
    await axios.post("/api/controller/seeker/verifyotp",{phoneNo:phoneRef.current.value}).then((res)=>{
    ...
    <ApexChartWrapper>
      <Button variant="contained" color="primary" sx={{flexGrow: 1}} onClick= {clickHanlder} >
            ...
            <TextField
              ...
              type="Number"
            />
      <Button variant="contained" color="primary" sx={{flexGrow: 1}} onClick= {clickHanlder1} >
              verify
            </Button>
