...
export default async function handler(req,res){
    ...
    // if(session.user.role==="institute"){
        // data=await ApplySeeker.find({seekerID:session.user.id,email:session.user.email,status:"meet"})
    // }
    // else{
         data=await ApplySeeker.find({agencyID:session.user.id,status:"meet"});
    // }
    console.log(data)
    // if(data[0]){
